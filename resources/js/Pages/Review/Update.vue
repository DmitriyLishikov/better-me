<template>
    ...
    <h2 v-text="'Update a review for ' + review.book.title"
        class="font-semibold text-xl text-gray-800 leading-tight">
    </h2>
    <form class="flex flex-col w-1/2" method="post" :action="route('reviews.update', {review: review.id})">
        ...
        <div class="flex flex-col m-2">
            ...
            <select name="rate" required>
                <option value="1" :selected="review.rate === 1">1</option>
                <option value="2" :selected="review.rate === 2">2</option>
                <option value="3" :selected="review.rate === 3">3</option>
                <option value="4" :selected="review.rate === 4">4</option>
                <option value="5" :selected="review.rate === 5">5</option>
            </select>
        </div>
        <div class="flex flex-col m-2">
            <label>Text review</label>
            <textarea :value="review ? review.content : ''" required name="content"></textarea>
        </div>
